...
namespace Pvr_UnitySDKAPI
{
    ...
    public struct Controller
    {
        ...
        public static SwipeDirection UPvr_GetTouchPadDirection(int hand,out string p)
        {
            //在这个小范围内不触发事件
            float eps = 0.001f;
            Vector2 offset = new Vector2(128, 128);
            Vector2 pos = UPvr_GetTouchPadPosition(hand) - offset;
            float x = pos.y, y = pos.x;
            p = x.ToString() + " " + y.ToString();
            if (Mathf.Abs(pos.x) <= eps && Mathf.Abs(pos.y) <= eps) return SwipeDirection.No;
            if (x > 0 && (x > Mathf.Abs(y))) return SwipeDirection.SwipeRight;
            if (x < 0 && (Mathf.Abs(x) > Mathf.Abs(y))) return SwipeDirection.SwipeLeft;
            if (y > 0 && (y > Mathf.Abs(x))) return SwipeDirection.SwipeUp;
            if (y < 0 && (Mathf.Abs(y) > Mathf.Abs(x))) return SwipeDirection.SwipeDown;
